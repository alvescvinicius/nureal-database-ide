...
- **Chat com IA (Ollama local)**: janela não-modal (ícone de balão de fala na barra de
  ferramentas) com streaming, histórico persistente por conversa e tool-calling (a IA
  pode listar tabelas/descrever colunas do banco conectado). Requer um Ollama já
  instalado e rodando (`ollama serve`) na máquina do usuário — configurável em ⚙ dentro
  da janela do chat (Base URL, modelo, temperatura, streaming).
