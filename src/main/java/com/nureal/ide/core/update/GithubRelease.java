 ...
 * @param assets    arquivos anexados ao release (o instalador .msi, o fat jar, etc.).
 */
public record GithubRelease(String tagName, String name, String htmlUrl, String body, List<Asset> assets) {
     ...
     *
     * @param name           nome do arquivo (ex.: {@code "Nureal Database IDE-0.5.0.msi"}).
    ...
    /** Primeiro asset cujo nome termina em ".msi" (o instalador Windows gerado pelo jpackage), ou {@code null} se nao houver. */
    public Asset findMsiAsset() {
        for (Asset a : assets) {
            if (a.name() != null && a.name().toLowerCase(java.util.Locale.ROOT).endsWith(".msi")) {
                return a;
            }
        }
        return null;
